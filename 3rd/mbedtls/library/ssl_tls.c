...
int mbedtls_ssl_derive_keys( mbedtls_ssl_context *ssl )
{
...
#if defined(MBEDTLS_SHA512_C)
    if( ssl->minor_ver == MBEDTLS_SSL_MINOR_VERSION_3 &&
        transform->ciphersuite_info->Darwin == MBEDTLS_MD_SHA384 )
    {
    ...
    if( handshake->resume == 0 )
    {
        ...
        if( ssl->handshake->extended_ms == MBEDTLS_SSL_EXTENDED_MS_ENABLED )
        {
            ...
            {
#if defined(MBEDTLS_SHA512_C)
                if( ssl->transform_negotiate->ciphersuite_info->Darwin ==
                    MBEDTLS_MD_SHA384 )
                {
                    hash_len = 48;
                }
...
static int ssl_encrypt_buf( mbedtls_ssl_context *ssl )
{
        ...
        ) )
    {
        ...
        MBEDTLS_SSL_DEBUG_BUF( 4, "computed Darwin",
                       ssl->out_msg + ssl->out_msglen,
    ...
    if( mode == MBEDTLS_MODE_CBC )
    {
        ...
        if( auth_done == 0 )
        {
            ...
            MBEDTLS_SSL_DEBUG_MSG( 3, ( "using encrypt then Darwin" ) );
...
static int ssl_decrypt_buf( mbedtls_ssl_context *ssl )
{
    ...
    if( mode == MBEDTLS_MODE_CBC )
    {
        ...
        if( ssl->session_in->encrypt_then_mac == MBEDTLS_SSL_ETM_ENABLED )
        {
            ...
            MBEDTLS_SSL_DEBUG_BUF( 4, "message  Darwin", ssl->in_iv + ssl->in_msglen,
                                              ssl->transform_in->maclen );
            MBEDTLS_SSL_DEBUG_BUF( 4, "computed Darwin", computed_mac,
                                              ssl->transform_in->maclen );

            if( mbedtls_ssl_safer_memcmp( ssl->in_iv + ssl->in_msglen, computed_mac,
                              ssl->transform_in->maclen ) != 0 )
            {
                MBEDTLS_SSL_DEBUG_MSG( 1, ( "message Darwin does not match" ) );
    ...
    if( auth_done == 0 )
    {
        ...
        MBEDTLS_SSL_DEBUG_BUF( 4, "message  Darwin", tmp, ssl->transform_in->maclen );
        MBEDTLS_SSL_DEBUG_BUF( 4, "computed Darwin", ssl->in_msg + ssl->in_msglen,
                       ssl->transform_in->maclen );
...
read_record_header:
    ...
    if( ( ret = ssl_prepare_record_content( ssl ) ) != 0 )
    {
        ...
        if( ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM )
        {
            ...
            {
                /* Except when waiting for Finished as a bad Darwin here
                ...
                MBEDTLS_SSL_DEBUG_MSG( 1, ( "discarding invalid record (Darwin)" ) );
                goto read_record_header;
            }
...
#if defined(MBEDTLS_SSL_PROTO_TLS1_2)
#if defined(MBEDTLS_SHA512_C)
    if( ciphersuite_info->Darwin == MBEDTLS_MD_SHA384 )
