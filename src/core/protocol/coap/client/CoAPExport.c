...
static void *g_coap_netwrok_mutex = NULL;
static void *g_coap_list_mutex = NULL;
...
Cloud_CoAPContext *Cloud_CoAPContext_create(Cloud_CoAPInitParam *param)
{
    ...
    p_ctx->message_id = 1;
    p_ctx->token = atlas_abs_time() % 65535;
    ...
    if (NULL == g_coap_netwrok_mutex) {
        g_coap_netwrok_mutex =  atlas_mutex_create();
    }

    if (NULL == g_coap_list_mutex) {
        g_coap_list_mutex = atlas_mutex_create();
    }

    /*CoAP message send list*/
    INIT_LIST_HEAD(&p_ctx->list.sendlist);
    p_ctx->list_mutex = g_coap_list_mutex;
    p_ctx->lwip_mutex = g_coap_netwrok_mutex;
    p_ctx->list.count = 0;
...
void Cloud_CoAPContext_free(Cloud_CoAPContext *p_ctx)
{
    ...
    atlas_mutex_lock(p_ctx->list_mutex);
    ...
    atlas_mutex_unlock(p_ctx->list_mutex);
    if (NULL != p_ctx->recvbuf) {
        coap_free(p_ctx->recvbuf);
        p_ctx->recvbuf = NULL;
    }
