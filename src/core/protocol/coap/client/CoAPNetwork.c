...
unsigned int Cloud_CoAPNetwork_write(coap_network_t *p_network, void *mutex,
                                     const unsigned char   *p_data,
                                     unsigned int           datalen)
{
    int rc = COAP_ERROR_WRITE_FAILED;
    uint64_t start = 0, end = 0;
#define  C_PRINTF_UDP_TIMEOUT     (1000)
#ifdef COAP_DTLS_SUPPORT
    if (COAP_ENDPOINT_DTLS == p_network->ep_type) {
        ...
        if (C_PRINTF_UDP_TIMEOUT + start < end) {
            printf("--->> udp send %d bytes more than %d ms!\n",  datalen,  (uint32_t)(end - start));
        }
...
int Cloud_CoAPNetwork_read(coap_network_t *network, void * mutex, unsigned char  *data,
...
#endif
        memset(data, 0x00, datalen);
        len = HAL_UDP_SELECT((intptr_t)network->context, timeout);
        if (0 == len) {
            atlas_mutex_lock(mutex);
            len = HAL_UDP_READ((intptr_t)network->context,
