                             ...
                             httpclient_data_t *client_data)
{
    ...
    send_buf = atlas_malloc(HTTPCLIENT_SEND_BUF_SIZE);
    buf = atlas_malloc(HTTPCLIENT_SEND_BUF_SIZE);
    if (!send_buf || !buf) {
        httpc_err("malloc buf and send buf failed");
        atlas_free(send_buf);
        atlas_free(buf);
        return ERROR_NO_MEM;
    }
                                       ...
                                       httpclient_data_t *client_data)
{
    ...
    while (NULL == (ptr_body_end = strstr(data, "\r\n\r\n"))) {
        ...
        // if rsp < max_len  must be timeout
        ret = _http_recv(client, data + len, 8, &new_trf_len, iotx_time_left(&timer));
        if (ret == ERROR_HTTP_CONN) {
            return ret;
        }
        len += new_trf_len;
    ...
    if (client->response_code == 301 || client->response_code == 302) {
        if (NULL != (tmp_ptr = strstr(data, "Location:"))) {
            if (NULL != (ptr_body_end = strstr(tmp_ptr, "\r\n")) ) {
                tmp_ptr += sizeof("Location:");
                if (client_data->response_buf_len < tmp_ptr - ptr_body_end) {
                    return ERROR_BUFF_OVERFLOW;
                }
                strncpy(client_data->response_buf, tmp_ptr, ptr_body_end - tmp_ptr);
                client_data->response_buf[ptr_body_end - tmp_ptr] = '\0';
                client_data->response_buf_len = strlen(client_data->response_buf);
                return SUCCESS_RETURN;
            } else {
                httpc_err("Could not parse header, not found \r\n");
                return SUCCESS_RETURN;
            }
        } else {
            httpc_err("Could not parse header, not found location");
            return ERROR_HTTP;
        }
    }
...
int httpclient_recv_response(httpclient_t *client, uint32_t timeout_ms, httpclient_data_t *client_data)
{
    ...
    buf = atlas_malloc(HTTPCLIENT_READ_BUF_SIZE);
    if (!buf) {
        return ERROR_NO_MEM;
    }
                      ...
                      HTTPCLIENT_REQUEST_TYPE method, httpclient_data_t *client_data)
{
    ...
    host = atlas_malloc(HTTPCLIENT_MAX_URL_LEN);
    path = atlas_malloc(HTTPCLIENT_MAX_URL_LEN);
    if (!host || !path) {
        httpc_err("malloc host or path failed");
        atlas_free(host);
        atlas_free(path);
        return ERROR_NO_MEM;
    }

    memset(host, 0, HTTPCLIENT_MAX_URL_LEN);
    ...
    if ((uintptr_t)(-1) == client->net.handle) {
        ...
        ret = iotx_net_init(&client->net, host, port, ca_crt, 0);
        if (0 != ret) {
            atlas_free(host);
            atlas_free(path);
            return ret;
...
{
    iotx_time_t timer;
    uint64_t start_send = atlas_boot_uptime();
    client->net.handle = -1;
    ...
    httpc_info("http send url=%s cost=%llu success\n", url, atlas_boot_uptime() - start_send);
    ...
    httpclient_close(client); //@todo 支持缓存传输
