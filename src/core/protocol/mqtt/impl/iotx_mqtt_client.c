...
static void iotx_mc_set_client_state(iotx_mc_client_t *pClient, iotx_mc_state_t newState)
{
    ...
    if (newState == IOTX_MC_STATE_DISCONNECTED || newState == IOTX_MC_STATE_CONNECTED) {
        pClient->last_receive_packet = 0;
        pClient->last_send_packet  = 0;
    }
...
static int iotx_mc_init(iotx_mc_client_t *pClient, iotx_mqtt_param_t *pInitParams)
{
    ...
    pClient->last_send_packet = 0;
    pClient->last_receive_packet = 0;
    pClient->last_network_check_time = 0;
    ...
    rc = iotx_net_init(&pClient->ipstack, pInitParams->host, pInitParams->port, pInitParams->pub_key, 1);
...
#endif
        mqtt_err("mqtt read buffer is too short, mqttReadBufLen : %d, remainDataLen : %d", c->buf_size_read, rem_len);
...
#define RETRY_INTV_PERIOD   (2000)
    ...
    mqtt_info("connect params: MQTTVersion=%d, clientID=%s, keepAliveInterval=%d, username=%s",
              ...
              pClient->connect_data.keepAliveInterval * 2,
    ...
    pClient->keepalive_probes = 0;
    pClient->last_network_check_time = atlas_boot_uptime();
...
static int iotx_mc_handle_recv_SUBACK(iotx_mc_client_t *c)
{
    ...
    if (rc < 0) {
        mqtt_err("Sub ack packet error, rc = MQTTDeserialize_suback() = %d msgid=%d", rc, mypacketid);
...
static int iotx_mc_cycle(iotx_mc_client_t *c, iotx_time_t *timer)
{
    ...
    mqtt_debug("iotx mc read packet exp_time=%u", timer->time);
    /* read the socket, see what work is due */
    rc = iotx_mc_read_packet(c, timer, &packetType);
    mqtt_debug("iotx mc read packet success exp_time=%u", timer->time);
    if (rc != SUCCESS_RETURN) {
        HAL_MutexLock(c->lock_read_buf);
        _reset_recv_buffer(c);
        HAL_MutexUnlock(c->lock_read_buf);
    ...
    HAL_MutexLock(c->lock_generic);
    c->keepalive_probes = 0;
    c->last_receive_packet = atlas_boot_uptime();
    HAL_MutexUnlock(c->lock_generic);
    HAL_MutexLock(c->lock_read_buf);
    switch (packetType) {
        case CONNACK: {
...
void _mqtt_cycle(void *client)
{
    ...
    mqtt_debug("mqtt cycle time=%d", pClient->cycle_timeout_ms);
    do {
        unsigned int left_t;
    ...
    mqtt_debug("mqtt cycle time=%d out", pClient->cycle_timeout_ms);
}
...
static int iotx_mc_keepalive_sub(iotx_mc_client_t *pClient)
{
    ...
    if(pClient->last_receive_packet < pClient->last_send_packet
    && pClient->last_receive_packet + CONFIG_MQTT_RECEIVE_PACKAGE_TIMEOUT <= atlas_boot_uptime()) {
        mqtt_err("Receive nothing more than %d, reconnect mqtt", CONFIG_MQTT_RECEIVE_PACKAGE_TIMEOUT);
        iotx_mc_set_client_state(pClient, IOTX_MC_STATE_DISCONNECTED);
        return rc;
    }

    atlas_check_network_disconnect(pClient->last_network_check_time, &is_disconnect);
    if (is_disconnect) {
        mqtt_err("Receive network disconnect reconnect mqtt");
        pClient->last_network_check_time = atlas_boot_uptime();
        iotx_mc_set_client_state(pClient, IOTX_MC_STATE_DISCONNECTED);
        return rc;
    }
...
static int iotx_mc_handle_reconnect(iotx_mc_client_t *pClient)
{
    ...
    if (CNIOT_STATUS_CODE_OK != atlas_get_network_status(&status))  {
        return FAIL_RETURN;
    }
    if (status != NETWORK_STATUS_ONLINE) {
        printf("Waiting to reconnect,network offline\n");
        return FAIL_RETURN;
    }
...

{
    ...
    mqtt_debug("start to get lock to alloc buff and write len=%d", topic_msg->payload_len);
    HAL_MutexLock(c->lock_list_pub);
    HAL_MutexLock(c->lock_write_buf);
...
int wrapper_mqtt_set_send_time(void * client) {
    iotx_mc_client_t *c = (iotx_mc_client_t *)client;
    if (!client) {
        return SUCCESS_RETURN;
    }
    ...
    HAL_MutexLock(c->lock_generic);
    if (c->last_send_packet <= c->last_receive_packet) {
        c->last_receive_packet = atlas_boot_uptime();
    }
    c->last_send_packet = atlas_boot_uptime();
    HAL_MutexUnlock(c->lock_generic);
    return SUCCESS_RETURN;
}
