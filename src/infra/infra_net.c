...
static int connect_ssl(utils_network_pt pNetwork)
{
    ...
    if ((uintptr_t)(-1) != (pNetwork->handle = (intptr_t) HAL_SSL_Establish(
            pNetwork->pHostAddress,
            pNetwork->port,
            pNetwork->ca_crt,
            pNetwork->ca_crt_len + 1, pNetwork->ip_flag))) {
        return 0;
    }
    else {
...
static int connect_tcp(utils_network_pt pNetwork)
{
    ...
    pNetwork->handle = HAL_TCP_Establish(pNetwork->pHostAddress, pNetwork->port, pNetwork->ip_flag);
    if (pNetwork->handle == (uintptr_t)(-1)) {
        return -1;
    }
...
{
    int     ret = 0;
    // 读数据在主线程，锁针对的是写数据和连接断开,如果读加锁，会导致业务rt变高.
    if (NULL == pNetwork || (uintptr_t)(-1) == pNetwork->handle) {
        return -1;
    }
...
{
    int     ret = 0;
    if (NULL == pNetwork || NULL == pNetwork->lock) {
        return -1;
    }
    atlas_mutex_lock(pNetwork->lock);
    if ((uintptr_t)(-1) == pNetwork->handle) {
        atlas_mutex_unlock(pNetwork->lock);
        return -1;
    }
    if (NULL != pNetwork->ca_crt) {
...
int iotx_net_init(utils_network_pt pNetwork, const char *host, uint16_t port, const char *ca_crt, uint16_t flag)
{
    if (!pNetwork || !host) {
    ...
    pNetwork->lock = atlas_mutex_create();
...
void iotx_net_exit(utils_network_pt pNetwork) {
    if (NULL == pNetwork || NULL == pNetwork->lock) {
        return;
    }
    atlas_mutex_destroy(pNetwork->lock);
    pNetwork->lock = NULL;
}
