...
uintptr_t atlas_tcp_enstablish(const char *host, uint16_t port, uint16_t flag)
{
    ...
    atlas_snprintf(service, 6, "%u", port);
    logger_debug("--->>establish tcp connection with server(host='%s', port=[%s])\n", host, service);

    rc = getaddrinfo(host, service, &hints, &addrInfoList);
    if (rc != 0) {
        logger_err("getaddrinfo error[%d], res: %d, host: %s, port: %s\n", dns_retry, rc, host, service);
        return -1;
    }
...
int32_t atlas_tcp_write(uintptr_t fd, const char *buf, uint32_t len, uint32_t timeout_ms)
{
    ...
    do {
        ...
        if (ret > 0) {
            once_len = len - len_sent;
            if (once_len >= 1024) {
                once_len = 1024;
            }
            ret = send(tcp_fd, buf + len_sent, once_len, 0);
            if (ret > 0) {
                len_sent += ret;
            } else if (0 == ret) {
    ...
    do {
        ...
        if (ret > 0) {
            ...
            if (ret > 0) {
                ...
                logger_err("connection is closed\n");
                err_code = -1;
                break;
            } else {
                if (EINTR == errno) {
