...
#define NETWORK_ADDR_LEN    (16)
    ...
    atlas_snprintf(port_ptr, 6, "%u", port);
    ...
    for (ainfo = res; ainfo != NULL; ainfo = ainfo->ai_next) {
        if (AF_INET == ainfo->ai_family) {
            ...
            fprintf(stderr, "The host IP %u, port is %d\r\n", sa->sin_addr.s_addr, ntohs(sa->sin_port));
...
int atlas_udp_write(intptr_t p_socket,
...
int atlas_udp_read_timeout(intptr_t p_socket,
...
int atlas_udp_connect(intptr_t sockfd,
                    const char *host,
                    unsigned short port)
{
    int                     rc = -1;
...
int atlas_udp_recvfrom(intptr_t sockfd,
                     void *remote,
                     unsigned char *p_data,
                     unsigned int datalen,
    ...
    fd_set read_fds;
    NetworkAddr *p_remote = (void *)remote;
