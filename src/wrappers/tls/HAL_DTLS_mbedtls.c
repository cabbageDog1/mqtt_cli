...
void *_DTLSCalloc_wrapper(size_t n, size_t size)
{
    ...
    buf = atlas_malloc(n * size + sizeof(mbedtls_mem_info_t));
    if (NULL == buf) {
        return NULL;
    } else {
...
dtls_session_t *_DTLSSession_init()
{
    ...
    if (NULL != p_dtls_session) {
        ...
        //mbedtls_entropy_init(&p_dtls_session->entropy); //@todo
...
unsigned int _DTLSSession_deinit(dtls_session_t *p_dtls_session)
{
    ...
    if (p_dtls_session != NULL) {
        ...
        //mbedtls_entropy_free(&p_dtls_session->entropy); // @todo
...
DTLSContext *HAL_DTLSSession_create(coap_dtls_options_t *p_options)
{
    ...
    if (NULL != p_dtls_session) {
        mbedtls_ssl_config_init(&p_dtls_session->conf);
        result = mbedtls_ctr_drbg_seed(&p_dtls_session->ctr_drbg, NULL, &p_dtls_session->entropy,
                                       (const unsigned char *)"IoTx",
                                       strlen("IoTx")); // @todo NULL ->mbedtls_entropy_func
        if (0 !=  result) {
        ...
        atlas_snprintf(port, 6, "%d", p_options->port);
